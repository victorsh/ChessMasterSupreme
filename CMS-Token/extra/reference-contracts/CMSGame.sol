...
contract ChessPieceGame is Ownable {
  ...
  uint constant minPiecesToken = 5 ether;
  ...
  constructor(CMSToken _cMSToken) public { 
    cMSToken = _cMSToken;
  }
  ...
  function createSingleGame(uint _tokenId) payable public returns (uint256) {
    ...
    require(address(this).balance >= minPiecesToken, "");
    require(cMSToken.ownerOf(_tokenId) == msg.sender, "");

    uint userTokenNumber;
    uint contractTokenNumber;
    (userTokenNumber, , ,) = cMSToken.getTokenProperty(_tokenId);
    (contractTokenNumber, , ,) = cMSToken.getTokenProperty(rand(0, cMSToken.getPiecesLength()));
